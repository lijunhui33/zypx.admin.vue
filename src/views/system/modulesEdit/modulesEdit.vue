...
export default {
	...
	created(){
		if(this.$route.query.q){
			...
			this.moduleDetail(obj.id)
		}
	},
	methods: {
		async moduleDetail(id){
			const data = await adminJs.moduleDetail({id:id,find_type:'id,name'},helper.adminHeaders())
			console.log(data)
			if(data.code === 0){
				this.form.setFieldsValue({
					name:data.data.name,
					description:data.data.description,
					gender:data.data.user_type_name,
					path:data.data.url
				})
				this.selectId = data.data.user_type_id
		...
		async add(values){
			...
			const data = await adminJs.putModule(obj,helper.adminHeaders())
			if(data.code === 0){
