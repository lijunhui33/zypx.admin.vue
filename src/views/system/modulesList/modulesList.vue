<template>
	<div style="margin: 20px;">
		<a-card title="模块列表" style="min-height: calc(100vh - 160px);">
			<a-button type="primary" class="addInp" @click="add">新增模块</a-button>
...
export default {
	...
	methods: {
		async moduleList() {
			const data = await adminJs.modileListPage({page_size:10,page_number:this.current}, helper.adminHeaders());
			if (data.code === 0) {
				...
				if(data.data.rows.length === 0){
					this.$router.push('(`/system/module/list/1')
				}
			}
		},
		deleteMessage(text) {
		...
		add() {
			this.$router.push('/system/module/add');
		},
		paginationChange(index){
			this.$router.push(`/system/module/list/${index}`)
		},
		edit(text) {
			this.$router.push({
