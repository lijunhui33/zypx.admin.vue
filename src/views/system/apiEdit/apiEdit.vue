...
export default {
	...
	methods: {
		async apiDetail(id){
			const data = await adminJs.apiDetail({id:id},helper.adminHeaders())
			if(data.code === 0){
				this.form.setFieldsValue({
					name:data.data.name,
					...
					path:data.data.url,
					methods:data.data.http_method,
					system_module_name:data.data.system_module_name
				})
		...
		async addApi(values) {
			...
			const data = await adminJs.putApi(obj,helper.adminHeaders())
			if(data.code === 0){
